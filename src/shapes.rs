...
impl DrawHandle<'_> {
    ...
    #[inline]
    pub fn set_shapes_texture(&self, texture: impl Into<WeakTexture>, source: Rectangle) {
        unsafe { ffi::SetShapesTexture(*texture.into().get_ffi(), source) }
    }

    /// Draw a single pixel.
    #[inline]
    pub fn pixel(&self, pos_x: f32, pos_y: f32, color: Color) {
        self.pixel_v(vec2(pos_x, pos_y), color)
    }
    /// Draw a single pixel.
    #[inline]
    pub fn pixel_v(&self, pos: Vector2, color: Color) {
        unsafe { ffi::DrawPixelV(pos, color) }
    }
    /// Draw a line.
    #[inline]
    pub fn line(&self, start_x: f32, start_y: f32, end_x: f32, end_y: f32, color: Color) {
        self.line_v(vec2(start_x, start_y), vec2(end_x, end_y), color)
    }
    /// Draw a line.
    #[inline]
    pub fn line_v(&self, start: Vector2, end: Vector2, color: Color) {
        unsafe { ffi::DrawLineV(start, end, color) }
    }
    /// Draw a line with thickness.
    #[inline]
    pub fn line_ex(&self, start: Vector2, end: Vector2, thick: f32, color: Color) {
    ...
    #[inline]
    pub fn line_strip(&self, points: &[Vector2], color: Color) {
    ...
    #[inline]
    pub fn line_bezier(&self, start: Vector2, end: Vector2, thick: f32, color: Color) {
        unsafe { ffi::DrawLineBezier(start, end, thick, color) }
    }
    /// Draw a filled circle.
    #[inline]
    pub fn circle(&self, center_x: f32, center_y: f32, radius: f32, color: Color) {
        self.circle_v(vec2(center_x, center_y), radius, color)
    }
    /// Draw part of a filled circle.
    /// Angles are in radians.
    #[inline]
    pub fn circle_sector(&self, center: Vector2, radius: f32, start_angle: f32, end_angle: f32, segments: i32, color: Color) {
    ...
    /// Angles are in radians.
    #[inline]
    pub fn circle_sector_lines(&self, center: Vector2, radius: f32, start_angle: f32, end_angle: f32, segments: i32, color: Color) {
        unsafe { ffi::DrawCircleSectorLines(center, radius, start_angle.to_degrees(), end_angle.to_degrees(), segments, color) }
    }
    /// Draw a gradient filled circle.
    #[inline]
    pub fn circle_gradient(&self, center: Vector2, radius: f32, color1: Color, color2: Color) {
        unsafe { ffi::DrawCircleGradient(center.x as i32, center.y as i32, radius, color1, color2) }
    }
    /// Draw a filled circle.
    #[inline]
    pub fn circle_v(&self, center: Vector2, radius: f32, color: Color) {
        unsafe { ffi::DrawCircleV(center, radius, color) }
    }
    /// Draw the outline of a circle.
    #[inline]
    pub fn circle_lines(&self, center_x: f32, center_y: f32, radius: f32, color: Color) {
        self.circle_lines_v(vec2(center_x, center_y), radius, color)
    }
    /// Draw the outline of a circle.
    #[inline]
    pub fn circle_lines_v(&self, center: Vector2, radius: f32, color: Color) {
        unsafe { ffi::DrawCircleLinesV(center, radius, color) }
    }
    /// Draw a filled ellipse.
    #[inline]
    pub fn ellipse(&self, center: Vector2, radius_h: f32, radius_v: f32, color: Color) {
        unsafe { ffi::DrawEllipse(center.x as i32, center.y as i32, radius_h, radius_v, color) }
    }
    /// Draw the outline of an ellipse.
    #[inline]
    pub fn ellipse_lines(&self, center: Vector2, radius_h: f32, radius_v: f32, color: Color) {
        unsafe { ffi::DrawEllipse(center.x as i32, center.y as i32, radius_h, radius_v, color) }
    }
    /// Draw a filled ring.
    #[inline]
    pub fn ring(&self, center: Vector2, inner_radius: f32, outer_radius: f32, color: Color) {
        self.ring_ex(center, inner_radius, outer_radius, 0.0, 2.0 * PI, 36, color)
    }
    /// Draw part of a filled ring.
    /// Angles are in radians.
    #[inline]
    pub fn ring_ex(&self, center: Vector2, inner_radius: f32, outer_radius: f32, start_angle: f32, end_angle: f32, segments: i32, color: Color) {
    ...
    #[inline]
    pub fn ring_lines(&self, center: Vector2, inner_radius: f32, outer_radius: f32, color: Color) {
        self.ring_lines_ex(center, inner_radius, outer_radius, 0.0, 2.0 * PI, 36, color)
    }
    /// Draw part of the outlines of a ring.
    /// Angles are in radians.
    #[inline]
    pub fn ring_lines_ex(&self, center: Vector2, inner_radius: f32, outer_radius: f32, start_angle: f32, end_angle: f32, segments: i32, color: Color) {
        unsafe { ffi::DrawRingLines(center, inner_radius, outer_radius, start_angle.to_degrees(), end_angle.to_degrees(), segments, color) }
    }
    /// Draw a filled rectangle.
    #[inline]
    pub fn rectangle(&self, pos_x: f32, pos_y: f32, width: f32, height: f32, color: Color) {
        self.rectangle_v(vec2(pos_x, pos_y), vec2(width, height), color)
    }
    /// Draw a filled rectangle.
    #[inline]
    pub fn rectangle_v(&self, pos: Vector2, size: Vector2, color: Color) {
        unsafe { ffi::DrawRectangleV(pos, size, color) }
    }
    /// Draw a filled rectangle.
    #[inline]
    pub fn rectangle_rec(&self, rec: Rectangle, color: Color) {
        unsafe { ffi::DrawRectangleRec(rec, color) }
    }
    /// Draw a rotated filled rectangle.
    #[inline]
    pub fn rectangle_pro(&self, rec: Rectangle, origin: Vector2, rotation: f32, color: Color) {
        unsafe { ffi::DrawRectanglePro(rec, origin, rotation, color) }
    }
    /// Draw a horizontal-gradient-filled rectangle.
    /// Gradient goes from left to right.
    #[inline]
    pub fn rectangle_gradient_h(&self, rec: Rectangle, left: Color, right: Color) {
    ...
    #[inline]
    pub fn rectangle_gradient_v(&self, rec: Rectangle, bottom: Color, top: Color) {
    ...
    #[inline]
    pub fn rectangle_gradient_ex(&self, rec: Rectangle, top_left: Color, bot_left: Color, bot_right: Color, top_right: Color) {
        unsafe { ffi::DrawRectangleGradientEx(rec, top_left, bot_left, bot_right, top_right) }
    }
    /// Draw the outline of a rectangle.
    #[inline]
    pub fn rectangle_lines(&self, pos_x: f32, pos_y: f32, width: f32, height: f32, color: Color) {
        unsafe { ffi::DrawRectangleLines(pos_x as i32, pos_y as i32, width as i32, height as i32, color) }
    }
    /// Draw the outline of a rectangle.
    #[inline]
    pub fn rectangle_lines_v(&self, pos: Vector2, size: Vector2, color: Color) {
        self.rectangle_lines(pos.x, pos.y, size.x, size.y, color)
    }
    /// Draw the outline of a rectangle.
    #[inline]
    pub fn rectangle_lines_rec(&self, rec: Rectangle, color: Color) {
        self.rectangle_lines(rec.x, rec.y, rec.width, rec.height, color)
    }
    /// Draw the outline of a rectangle with line thickness.
    #[inline]
    pub fn rectangle_lines_ex(&self, rec: Rectangle, thick: f32, color: Color) {
        unsafe { ffi::DrawRectangleLinesEx(rec, thick, color) }
    }
    /// Draw a filled rectangle with rounded corners.
    /// `roundness` goes from 0.0 to 1.0.
    #[inline]
    pub fn rectangle_rounded(&self, rec: Rectangle, roundness: f32, segments: i32, color: Color) {
    ...
    /// `roundness` goes from 0.0 to 1.0.
    #[inline]
    pub fn rectangle_rounded_lines(&self, rec: Rectangle, roundness: f32, segments: i32, thick: f32, color: Color) {
    ...
    #[inline]
    pub fn triangle(&self, v1: Vector2, v2: Vector2, v3: Vector2, color: Color) {
    ...
    #[inline]
    pub fn triangle_unordered(&self, mut v1: Vector2, v2: Vector2, mut v3: Vector2, color: Color) {
    ...
    #[inline]
    pub fn triangle_lines(&self, v1: Vector2, v2: Vector2, v3: Vector2, color: Color) {
    ...
    #[inline]
    pub fn triangle_lines_unordered(&self, mut v1: Vector2, v2: Vector2, mut v3: Vector2, color: Color) {
        // calculate the determinant of matrix [[x1, y1, 1], [x2, y2, 1], [x3, y3, 1]]
        let det = v3.x*(v1.y - v2.y) + v1.x*(v2.y - v3.y) + v2.x*(-v1.y + v3.y);
        // positive determinant -> counter-clockwise, negative determinant -> clockwise
        if det < 0.0 {
    ...
    #[inline]
    pub fn triangle_fan(&self, points: &[Vector2], color: Color) {
        // NOTE: The cast_mut is sound, as the raylib API doesn't modify the input in any way, it simply forgot the specify the parameter as const.
        // However, that does mean that raylib can later change internally to modify the input (though that would be very very naughty).
        // Thus, this function could become unsound in a future version of raylib, and must be closely monitored.
    ...
    /// Draws nothing if less than 3 points are given.
    #[inline]
    pub fn triangle_strip(&self, points: &[Vector2], color: Color) {
        // NOTE: The cast_mut is sound, as the raylib API doesn't modify the input in any way, it simply forgot the specify the parameter as const.
        // However, that does mean that raylib can later change internally to modify the input (though that would be very very naughty).
        // Thus, this function could become unsound in a future version of raylib, and must be closely monitored.
    ...
    #[inline]
    pub fn draw_poly(&self, center: Vector2, sides: usize, radius: f32, rotation: f32, color: Color) {
        unsafe { ffi::DrawPoly(center, sides as i32, radius, rotation, color) }
    }
    /// Draw the outline of a regular polygon (pentagon, hexagon, etc..).
    #[inline]
    pub fn draw_poly_lines(&self, center: Vector2, sides: usize, radius: f32, rotation: f32, color: Color) {
    ...
    #[inline]
    pub fn draw_poly_lines_ex(&self, center: Vector2, sides: usize, radius: f32, rotation: f32, thickness: f32, color: Color) {
...
impl DrawHandle<'_> {
    ...
    #[inline]
    pub fn draw_spline_linear(&self, points: &[Vector2], thickness: f32, color: Color) {
    ...
    #[inline]
    pub fn draw_spline_basis(&self, points: &[Vector2], thickness: f32, color: Color) {
        // SAFETY: points aren't modified
    ...
    #[inline]
    pub fn draw_spline_catmull_rom(&self, points: &[Vector2], thickness: f32, color: Color) {
        // SAFETY: points aren't modified
    ...
    #[inline]
    pub fn draw_spline_bezier_quadratic(&self, points: &[Vector2], thickness: f32, color: Color) {
        // SAFETY: points aren't modified
    ...
    #[inline]
    pub fn draw_spline_bezier_cubic(&self, points: &[Vector2], thickness: f32, color: Color) {
        // SAFETY: points aren't modified
        unsafe { ffi::DrawSplineBezierCubic(points.as_ptr().cast_mut(), points.len() as i32, thickness, color) }
    }
    /// Draw a single linear spline.
    #[inline]
    pub fn draw_spline_seg_linear(&self, p1: Vector2, p2: Vector2, thickness: f32, color: Color) {
        unsafe { ffi::DrawSplineSegmentLinear(p1, p2, thickness, color) }
    }
    /// Draw a single B-Spline.
    #[inline]
    pub fn draw_spline_seg_basis(&self, p1: Vector2, p2: Vector2, p3: Vector2, p4: Vector2, thickness: f32, color: Color) {
        unsafe { ffi::DrawSplineSegmentBasis(p1, p2, p3, p4, thickness, color) }
    }
    /// Draw a single Catmull-Rom spline.
    #[inline]
    pub fn draw_spline_seg_catmull_rom(&self, p1: Vector2, p2: Vector2, p3: Vector2, p4: Vector2, thickness: f32, color: Color) {
        unsafe { ffi::DrawSplineSegmentCatmullRom(p1, p2, p3, p4, thickness, color) }
    }
    /// Draw a single quadratic bezier spline.
    #[inline]
    pub fn draw_spline_seg_bezier_quad(&self, p1: Vector2, p2: Vector2, p3: Vector2, thickness: f32, color: Color) {
        unsafe { ffi::DrawSplineSegmentBezierQuadratic(p1, p2, p3, thickness, color) }
    }
    /// Draw a single cubic bezier spline.
    #[inline]
    pub fn draw_spline_seg_bezier_cubic(&self, p1: Vector2, p2: Vector2, p3: Vector2, p4: Vector2, thickness: f32, color: Color) {
